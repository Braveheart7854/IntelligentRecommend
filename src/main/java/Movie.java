...
//        JavaSparkContext sparkContext = new JavaSparkContext(new SparkConf().setAppName("SparkMysql").setMaster("local[5]"));
        ...
        //读取mysql数据
        JavaRDD<String[]> data = new SparkMysql().readMySQL(sqlContext);
        //停止SparkContext
//        sparkContext.stop();
...
//        System.out.println(data.count());
        ...
        // 加载评分数据
//        String path = "/Users/tongh/Movies/ratings.dat";
...
//                System.out.println(userId);
                ...
                double rating = Float.parseFloat(line[2]);
                int timestamp = (int) (Long.parseLong(line[3])%10);
...
//                int timestamp = (int) (Long.parseLong(fields[3])%10);
        ...
        JavaRDD<Integer> users = ratingsTrain_KV.map(new Function<Tuple2<Integer, Rating>, Integer>() {
            @Override
            public Integer call(Tuple2<Integer, Rating> v1) throws Exception {
                return v1._2.user();
            }
        });

        JavaRDD<Integer> movies = ratingsTrain_KV.map(new Function<Tuple2<Integer, Rating>, Integer>() {
            @Override
            public Integer call(Tuple2<Integer, Rating> v1) throws Exception {
                return v1._2.product();
            }
        });

        System.out.println("get " + ratingsTrain_KV.count() + " ratings from " + users.distinct().count() + "users on " + movies.distinct().count() + "movies");
//        System.exit(1);

        // 加载我的评分数据
        String mypath = "/Users/tongh/Movies/test.dat";
        ...
        for (int i = 0; i < ranks.size(); i++) {
            for (int j = 0; j < lambdas.size(); j++) {
                for (int k = 0; k < iters.size(); k++) {
                    //训练获得模型
                    MatrixFactorizationModel model = ALS.train(JavaRDD.toRDD(traningData_Rating), ranks.get(i), iters.get(j), lambdas.get(k));
                    ...
                    System.out.println("validation = " + validateRnse + " for the model trained with rank = " + ranks.get(i) + " lambda = " + lambdas.get(j) + " and numIter = " + iters.get(k));
        ...
        //加载电影数据
        String moviepath = "/Users/tongh/Movies/movies.dat";
        ...
        if (list != null && !list.isEmpty()) {
            ...
            movieList_Tuple.filter(new Function<Tuple3<Integer,String,String>, Boolean>() {
                ...
                public void call(Tuple3<Integer, String, String> t) throws Exception {
                    System.out.println("影片名：" + t._2());
                    System.out.println("影片类别：" + t._3());
                    System.out.println("-------------------------");
                }
            });
        }else{
